...
//====================================================================================================================================================
uint32_t VL53L0X::startSensor(void){
	writeReg(0x80, 0x01);
	writeReg(0xFF, 0x01);
	writeReg(0x00, 0x00);
	writeReg(0x91, stop_variable);
	...
	// Was hier aan het testen of ik die while weg mag laten, heb ook de timer if bij checksensordata aangepast, werkte daarvoor
	
	return timeout_start_ms;
}

uint8_t VL53L0X::checkSensorData(uint32_t timeOutStart){

	if ((readReg(RESULT_INTERRUPT_STATUS) & 0x07) == 0){
		if (io_timeout > 0 && (uint32_t)millis() > (timeOutStart + io_timeout)){
			return 2;
		}
		else{
			return 1;
...
uint16_t VL53L0X::leesSensor(void){
	uint16_t range = readReg16Bit(RESULT_RANGE_STATUS + 10);
